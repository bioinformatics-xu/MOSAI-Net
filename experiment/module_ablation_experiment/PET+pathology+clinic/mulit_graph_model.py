...
class Mulit_Graph_model:
    def __init__(self, pet_features, pathology_features, extra_features,
                 pet_adj, pathology_adj,extra_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
            pet_num_features=pet_features.size(1),pathology_num_features=pathology_features.size(1),extra_num_features=extra_features.size(1),
            num_classes=labels.max().item() + 1,
            pet_edge_index=pet_adj,pathology_edge_index=pathology_adj,extra_edge_index=extra_adj,
            pet_x=pet_features,pathology_x=pathology_features,extra_x=extra_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            optimiser.zero_grad()
            # 三个GCN和融合模块
            logits,self.fused_features, self.fused_adj = self.multimodal_gcn(
                self.pet_features, self.pathology_features,  self.extra_features,
                self.pet_adj, self.pathology_adj, self.extra_adj)
            ...
            loss = loss_cla + self.args.r1 * loss_Ncontrast + self.args.r2 * loss_fuse
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                _, fused_features_eval, fused_adj_eval = self.multimodal_gcn(self.pet_features, self.pathology_features,
                                                                             self.extra_features,
        ...
        with torch.no_grad():
            _, fused_features_final, fused_adj_final = self.multimodal_gcn(
                self.pet_features, self.pathology_features, self.extra_features,
                self.pet_adj, self.pathology_adj,self.extra_adj)
