...
class Multi_GNN_model(nn.Module):
    def __init__(self, pathology_num_features, gene_num_features,
                 num_classes, pathology_edge_index, gene_edge_index,
                pathology_x, gene_x,args):
    ...
    def forward(self, pathology_x, gene_x, pathology_edge_index, gene_edge_index):
        ...
        x_f = self.fuse([pathology_x_gcn, gene_x_gcn])
        # x_f = self.bottleneck(fusion_h_3)
        x_f_4 = torch.cat([pathology_x_gcn, gene_x_gcn, x_f], dim=-1)
        x_f = self.bottleneck(x_f_4)
        # 对每个节点进行分类
        logits = self.classifier(x_f)

        # 融合边索引 + 过滤
        fused_edge_index = self.merge_edge_indices(pathology_edge_index, gene_edge_index)
