...
class AutoDimFuse(nn.Module):
    """
    输入: 3 个同形张量 [B, D]（D 为特征维度）
    输出: [B, D]  每个维度独立地选 0/1/2 中分数最高的
    """
    def __init__(self, dim):
        super().__init__()
        # 为每个维度独立预测 3 类分数
        self.gate = nn.Sequential(
            nn.Linear(dim * 3, dim * 3),  # 先升维
            nn.ReLU(),
            nn.Linear(dim * 3, dim * 3)    # 输出 [B, D*3]
        )

    def forward(self, x_list, return_ratio=False):
        """
        x_list: [x0, x1, x2], 每个 [B, D]
        return_ratio: 是否返回模态使用比例（仅推理时有效）
        """
        assert len(x_list) == 3
        x0, x1, x2 = x_list                      # [B, D]
        z = torch.cat([x0, x1, x2], dim=-1)     # [B, 3D]
        logits = self.gate(z).view(x0.size(0), x0.size(1), 3)  # [B, D, 3]

        # =======================
        #   训练阶段：Gumbel-Softmax（可反向传播）
        # =======================
        if self.training:
            mask = F.gumbel_softmax(logits, tau=5.0, hard=True)  # [B, D, 3] one-hot
        else:
            # =======================
            #   推理阶段：argmax
            # =======================
            idx = torch.argmax(logits, dim=-1)                   # [B, D]
            mask = F.one_hot(idx, num_classes=3).float()         # [B, D, 3]
    ...
    def get_modality_usage_ratio(self, mask):
        """
        mask: [B, D, 3] one-hot
        返回：list 长度 3，分别是模态 0/1/2 被选中的比例
        """
        mask = mask.reshape(-1, 3)  # [B*D, 3]
        total = mask.size(0)
