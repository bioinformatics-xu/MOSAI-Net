...
parser.add_argument('--trans_dim', type=int, default=256, help='hidden dimension for transformer.')
...
parser.add_argument('--ft_size', type=int, default=256, help='feature size of each node.')
...
if __name__ == '__main__':
    ...
    # 设置路径
    graph_dir = r"D:\PycharmProjects\Recurrence_prediction_0906\experiment\module_ablation_experiment\pathology+gene+clinic\graph_structure"
    data_path = r"D:\PycharmProjects\Recurrence_prediction_0906\build_graph\data\data_0906.xlsx"

    # 加载数据
    X_pathology, X_gene, X_extra, pathology_edge_index, gene_edge_index, extra_edge_index, y = load_data(graph_dir, data_path)
    num_nodes = y.size(0)
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(np.arange(num_nodes), y_np)):
        ...
        embedder = Mulit_Graph_model(
            X_pathology, X_gene, X_extra,
            pathology_edge_index, gene_edge_index, extra_edge_index,
