...
class Mulit_Graph_model:
    def __init__(self, pathology_features, gene_features, extra_features,
                 pathology_adj,gene_adj,extra_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
           pathology_num_features=pathology_features.size(1),gene_num_features=gene_features.size(1),extra_num_features=extra_features.size(1),
            num_classes=labels.max().item() + 1,
           pathology_edge_index=pathology_adj,gene_edge_index=gene_adj,extra_edge_index=extra_adj,
           pathology_x=pathology_features,gene_x=gene_features,extra_x=extra_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            logits,self.fused_features, self.fused_adj = self.multimodal_gcn(
                 self.pathology_features, self.gene_features, self.extra_features,
                self.pathology_adj, self.gene_adj, self.extra_adj)
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                _, fused_features_eval, fused_adj_eval = self.multimodal_gcn( self.pathology_features,
