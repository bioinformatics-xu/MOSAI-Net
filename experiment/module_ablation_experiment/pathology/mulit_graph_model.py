...
class Mulit_Graph_model:
    def __init__(self, pathology_features,
                 pathology_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
            pathology_num_features=pathology_features.size(1),
            num_classes=labels.max().item() + 1,
            pathology_edge_index=pathology_adj,
            pathology_x=pathology_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            optimiser.zero_grad()
            # 三个GCN和融合模块
            logits,pathology_log,self.fused_features, self.fused_adj = self.multimodal_gcn(
                self.pathology_features,
                self.pathology_adj)
            ...
            # 计算损失
            pet_loss = F.nll_loss(pathology_log[self.idx_train], self.labels[self.idx_train])
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                _,_, fused_features_eval, fused_adj_eval = self.multimodal_gcn(self.pathology_features,
                                                                             self.pathology_adj)
