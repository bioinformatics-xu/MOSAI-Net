...
class Multi_GNN_model(nn.Module):
    def __init__(self, pathology_num_features, extra_num_features,
                 num_classes, pathology_edge_index, extra_edge_index,
                 pathology_x, extra_x, args):
        ...
        self.bottleneck = nn.Sequential(
            nn.Linear(args.hidden_dim*3, args.hidden_dim*3),
            nn.LayerNorm(args.hidden_dim*3),
        ...
        # 分类器
        self.classifier = nn.Linear(args.hidden_dim*3, num_classes)

    def forward(self, pathology_x, extra_x, pathology_edge_index, extra_edge_index):
        ...
        x_f = self.fuse([pathology_x_gcn, extra_x_gcn])
        # x_f = self.bottleneck(fusion_h_3)
        x_f_4 = torch.cat([pathology_x_gcn, extra_x_gcn, x_f], dim=-1)
        x_f = self.bottleneck(x_f_4)
        # 对每个节点进行分类
        logits = self.classifier(x_f)

        # 融合边索引 + 过滤
        fused_edge_index = self.merge_edge_indices(pathology_edge_index,extra_edge_index)

        return logits, x_f, fused_edge_index

    def merge_edge_indices(self, edge_index1, edge_index2):
        """
        ...
        # 合并四个模态的边
        fused_edge_index = torch.cat([edge_index1, edge_index2], dim=1)
