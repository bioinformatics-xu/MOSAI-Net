...
class Mulit_Graph_model:
    def __init__(self, pet_features, extra_features,
                 pet_adj,extra_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
            pet_num_features=pet_features.size(1),extra_num_features=extra_features.size(1),
            num_classes=labels.max().item() + 1,
            pet_edge_index=pet_adj,extra_edge_index=extra_adj,
            pet_x=pet_features,extra_x=extra_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            logits,self.fused_features, self.fused_adj = self.multimodal_gcn(
                self.pet_features, self.extra_features,
                self.pet_adj, self.extra_adj)
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                _, fused_features_eval, fused_adj_eval = self.multimodal_gcn(self.pet_features, self.extra_features,
                                                                             self.pet_adj, self.extra_adj)
