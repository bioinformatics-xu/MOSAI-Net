...
class Mulit_Graph_model:
    def __init__(self, pet_features, gene_features,
                 pet_adj, gene_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
            pet_num_features=pet_features.size(1),gene_num_features=gene_features.size(1),
            num_classes=labels.max().item() + 1,
            pet_edge_index=pet_adj,gene_edge_index=gene_adj,
            pet_x=pet_features,gene_x=gene_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            logits,self.fused_features, self.fused_adj = self.multimodal_gcn(
                self.pet_features,  self.gene_features,
                self.pet_adj, self.gene_adj)
