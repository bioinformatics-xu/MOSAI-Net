...
class Mulit_Graph_model:
    def __init__(self, pet_features,
                 pet_adj,
        ...
        self.multimodal_gcn = Multi_GNN_model(
            pet_num_features=pet_features.size(1),
            num_classes=labels.max().item() + 1,
            pet_edge_index=pet_adj,
            pet_x=pet_features,
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            optimiser.zero_grad()
            # 三个GCN和融合模块
            logits,pet_log,self.fused_features, self.fused_adj = self.multimodal_gcn(
                self.pet_features,
                self.pet_adj)
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                _,_, fused_features_eval, fused_adj_eval = self.multimodal_gcn(self.pet_features,
                                                                             self.pet_adj)
